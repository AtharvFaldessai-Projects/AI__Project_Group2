...
st.set_page_config(page_title="AI Study Architect", layout="wide")
...
if page == "Home":
    ...
    task_choice = st.selectbox("Enter task type:", task_options).lower()
    student_task_difficulty = st.slider(f"How difficult is this {task_choice}?", 1.0, 10.0, 5.0)

    col1, col2 = st.columns(2)
    with col1:
        time_unit = st.radio("Time Unit:", ["Minutes", "Hours"]).lower()
    with col2:
        time_input = st.number_input(f"Avg time spent on this {task_choice}:", min_value=0.0, value=60.0)
    ...
    if st.button("Generate Prediction"):
        ...
        st.session_state['shared_time'] = final_hrs
        st.session_state['shared_task'] = "Academic" # Defaulting for model 2
        ...
        st.write(f"Working Time: {(predicted_total_time * 0.75):.2f} {time_unit}")
